(() => {
  ...
  const lsKey = (login) => 'twn:sub:' + login;

  function syncLocalStorage() {
    try {
      const keep = new Set(mineChannels.map((c) => lsKey(c.login)));
  ...
  function channelRow(channel, actions) {
    ...
    login.textContent = 'twitch.tv/' + channel.login;
  ...
  function addButton(channel) {
    ...
    btn.type = 'button';
    if (subscribed.has(channel.login)) {
      ...
      btn.addEventListener('click', () => add(channel.login, btn));
    }
    return btn;
  }
  ...
  function render() {
    ...
    el.mine.replaceChildren(...mineChannels.map((c) => {
      ...
      test.addEventListener('click', () => sendTest(c.login, test));
      const remove = document.createElement('button');
      remove.type = 'button';
      remove.className = 'btn-remove';
      remove.textContent = 'Remove';
      remove.addEventListener('click', () => removeChannel(c.login, remove));
  ...
  async function refreshMine() {
    ...
    if (pushSub) {
      ...
      if (res.ok) {
        ...
        for (const c of mineChannels) subscribed.add(c.login);
      }
    }
    syncLocalStorage();
    render();
  ...
  async function add(login, btn) {
    btn.disabled = true;
    btn.textContent = 'Adding…';
    try {
      const cfgRes = await fetch('/api/config?channel=' + encodeURIComponent(login));
      ...
      const res = await fetch('/api/subscribe', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ channel: login, subscription: pushSub.toJSON() }),
      });
  ...
  async function removeChannel(login, btn) {
    btn.disabled = true;
    try {
      const res = await fetch('/api/unsubscribe', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ channel: login, endpoint: pushSub.endpoint }),
      });
  ...
  async function sendTest(login, btn) {
    btn.disabled = true;
